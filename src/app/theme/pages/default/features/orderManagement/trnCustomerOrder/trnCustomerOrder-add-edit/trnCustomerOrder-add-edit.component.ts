...
@Component({
  selector: "app-trnCustomerOrder-add-edit",
  templateUrl: "./trnCustomerOrder-add-edit.component.html",
  encapsulation: ViewEncapsulation.None,
})
export class TrnCustomerOrderAddEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.customerObj = {};
    this.customerObj.MstCustomerAddresses = [];
    let today = new Date();
    this.locationObj = {};
    this.disabled = false;
  ...
  getCustomerById(id) {
    ...
    this.customerService.getloggedInCustomerById(id).subscribe(
      results => {
        this.customerObj = results;
        this.customerObj.MstCustomerAddresses = results.mstCustomerAddresses;
        this.trnSalesOrderObj.customerId = results.id;
        if (!this.params)
          this.shippingAddressObj = _.find(this.customerObj.MstCustomerAddresses, ['isPrimary', true]);
  ...
  getTrnSalesOrderById(id) {
    Helpers.setLoading(true);
    this.trnSalesOrderService.getTrnSaleOrderByIdForCustomer(id).subscribe(
      results => {
  ...
  onSubmit({ value, valid }: { value: any, valid: boolean }) {
    ...
    this.trnSalesOrderObj.customerName = this.customerObj ? this.customerObj.name : '';
    if (this.trnSaleOrderItems.length == 0) {
  ...
  saveTrnSalesOrder(value) {
    ...
    if (this.params) {
      ...
      this.trnSalesOrderService.createTrnSaleOrderByCustomer(value)
        .subscribe(
        results => {
          this.params = null;
          this.messageService.addMessage({ severity: results.type.toLowerCase(), summary: results.type, detail: results.message });
