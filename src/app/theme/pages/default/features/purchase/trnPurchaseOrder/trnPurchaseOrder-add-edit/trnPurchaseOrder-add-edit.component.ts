...
export class TrnPurchaseOrderAddEditComponent implements OnInit {
  ...
  getTrnPurchaseOrderById(id) {
    ...
    this.trnPurchaseOrderService.getTrnPurchaseOrderById(id).subscribe(
      results => {
        ...
        this.trnPurchaseOrderObj.orderDate = new Date(
          this.trnPurchaseOrderObj.orderDate
        );
  ...
  addItemToList() {
    if (!this.categoryId) this.categoryIdError = true;
    else this.categoryIdError = false;

    if (!this.collectionId && this.categoryId != 7)
      this.collectionIdError = true;
    else this.collectionIdError = false;

    if (
      !this.shadeId &&
      (this.categoryId == 1 || this.categoryId == 5 || this.categoryId == 6)
    )
      this.shadeIdError = true;
    else this.shadeIdError = false;

    if (!this.matSizeId && this.categoryId == 4) this.matSizeIdError = true;
    else this.matSizeIdError = false;

    if (!this.fomSizeId && this.categoryId == 2) this.fomSizeIdError = true;
    else this.fomSizeIdError = false;

    if (!this.accessoryId && this.categoryId == 7) this.accessoryIdError = true;
    else this.accessoryIdError = false;
    ...
    else this.widthError = false;

    if (this.categoryId == 4 && this.matSizeId == -1 && !this.matThicknessId)
      this.matThicknessIdError = true;
    else this.matThicknessIdError = false;

    if (this.categoryId == 4 && this.matSizeId == -1 && !this.qualityId)
      this.qualityIdError = true;
    else this.qualityIdError = false;

    if (!this.orderQuantity) this.orderQuantityError = true;
    else this.orderQuantityError = false;

    if (
      this.orderQuantityError ||
      this.widthError ||
      this.matThicknessIdError ||
      this.fomSizeIdError ||
      this.matSizeIdError ||
      this.qualityIdError ||
      this.accessoryIdError ||
      this.lengthError ||
      this.shadeIdError ||
      this.collectionIdError ||
    ...
    if (this.trnPurchaseOrderItems.length > 0) {
      let poObj = _.find(this.trnPurchaseOrderItems, [
        "categoryId",
        this.categoryId
      ]);
      if (poObj != null) {
        if (
          this.accessoryId == poObj.accessoryId &&
          this.shadeId == poObj.shadeId &&
          this.fomSizeId == poObj.fomSizeId &&
          this.matSizeId == poObj.matSizeId &&
          this.matSizeCode == poObj.matSizeCode
        ) {
          this.messageService.addMessage({
            severity: "error",
            summary: "Error",
            detail: "Cannot add duplicate items."
          });
          return false;
        }
      }
    ...
    let accessoryObj = _.find(this.accessoryCodeList, [
      "value",
      this.accessoryId
    ]);
    ...
    this.trnPurchaseOrderObj.totalAmount = Math.round(
      this.trnPurchaseOrderObj.totalAmount + parseInt(this.amountWithGST)
    );
  ...
  onCancelItemDetails() {
    ...
    (this.amount = null), (this.tempPurchaseDiscount = 0);
    this.orderType = "";
    this.amountWithGST = "";
  }
  ...
  calculateProductStockDetails() {
    ...
    if (this.categoryId == 1 || this.categoryId == 5 || this.categoryId == 6) {
      ...
      this.rate = null;
      this.orderQuantity = null;
      (this.amount = null), (this.matSizeCode = null);
      this.orderType = "";
      ...
      this.matSizeCode = null;
      this.orderQuantity = null;
      (this.amount = null), (this.orderType = "");
      this.amountWithGST = null;
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          if (this.trnPurchaseOrderObj.totalAmount >= 0) {
            if (this.trnPurchaseOrderItems.length > 0) {
              this.trnPurchaseOrderObj.totalAmount =
                this.trnPurchaseOrderObj.totalAmount -
                this.trnPurchaseOrderItems[index].amountWithGST;
            } else {
              this.trnPurchaseOrderObj.totalAmount = 0;
            }
          }
  ...
  changeOrderType() {
    ...
    if (this.orderQuantity >= 50) {
      ...
      if (!this.orderQuantity) this.orderType = "";
      else this.orderType = "CL";
    }
    ...
    if (this.categoryId == 2) {
      ...
      this.amount = Math.round(
        this.amount - (this.amount * this.productDetails.purchaseDiscount) / 100
      );
      ...
      if (this.matSizeId != -1) {
        ...
        this.rate =
          ((this.length * this.width) / 1550.5) *
          this.productDetails.custRatePerSqFeet;
  ...
  onChangeCategory() {
    ...
    if (this.categoryId != null) {
      if (this.categoryId == 7) {
        if (this.trnPurchaseOrderObj.supplierId != null) {
          this.getAccessoryLookUpBySupplierId(
  ...
  onSubmit({ value, valid }: { value: any; valid: boolean }) {
    ...
    if (valid) {
      let supplierObj = _.find(this.supplierCodeList, [
        "value",
        this.trnPurchaseOrderObj.supplierId
      ]);
      let couierObj = _.find(this.courierList, [
        "value",
        this.trnPurchaseOrderObj.courierId
      ]);
      let shippingAddress = "";
  ...
  saveTrnPurchaseOrder(value) {
    ...
    Helpers.setLoading(true);
    if (this.params) {
      this.trnPurchaseOrderService.updateTrnPurchaseOrder(value).subscribe(
        results => {
          this.params = null;
          this.messageService.addMessage({
            severity: results.type.toLowerCase(),
      ...
      this.trnPurchaseOrderService.createTrnPurchaseOrder(value).subscribe(
        results => {
          this.params = null;
          this.messageService.addMessage({
            severity: results.type.toLowerCase(),
